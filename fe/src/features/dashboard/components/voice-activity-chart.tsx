...
const generateData = () => {
  ...
  for (let i = 30; i >= 1; i--) {
    ...
    const jointHealthSessions = Math.floor(totalSessions * (0.35 + Math.random() * 0.05)); // 35-40%
    const eyeHealthSessions = Math.floor(totalSessions * (0.30 + Math.random() * 0.05));  // 30-35%
    const weightManagementSessions = Math.floor(totalSessions * (0.20 + Math.random() * 0.05)); // 20-25%
    // Calculate purchased sessions (conversion rate around 20-30%)
    const purchasedSessions = Math.floor(totalSessions * (0.20 + Math.random() * 0.10));
