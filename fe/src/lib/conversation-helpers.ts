 ...
 */
export function detectPurchasedProduct(
...
export function generateTags(
  ...
  const hasProducts = messages.some(
    (msg) => msg.products && msg.products.length > 0
  )
  if (hasProducts) {
    tags.push('產品詢問')
  }
...
export function calculateEngagement(
  ...
  const userCharCount = userMessages.reduce(
    (sum, msg) => sum + (msg.content ? msg.content.length : 0),
    0
  )

  const assistantCharCount = assistantMessages.reduce(
    (sum, msg) => sum + (msg.content ? msg.content.length : 0),
    0
  )
