...
export const ConversationsProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const [conversionFilter, setConversionFilter] =
    useState<ConversionFilterType>('all')
  ...
  const convertedChats = conversations.filter(
    (chat) => chat.purchasedProduct
  ).length
  const conversionRate =
    totalChats > 0 ? ((convertedChats / totalChats) * 100).toFixed(1) : '0.0'
  ...
  useEffect(() => {
    ...
    async function fetchConversations() {
      try {
        ...
        const uiConversations: UIConversation[] = data
          .map((conv) => ({
            id: `chat-${conv.id}`,
        ...
        for (const conv of uiConversations) {
          ...
          try {
            ...
            const uiMessages: UIMessage[] = messages.map((msg) => ({
              ...
              content: msg.content,
              products: extractProductsFromMessage(
                msg.content
              ) as unknown as UIMessage['products'],
